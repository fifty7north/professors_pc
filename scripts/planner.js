...
gamePokemonData.sort((a, b) => {
  return (
    a[1][0]["regional_pokedex_id"][gameNumber] -
    b[1][0]["regional_pokedex_id"][gameNumber]
  );
});
...
gamePokemonData.forEach((pokemon) => {
  ...
  newPokemonEntryImage.setAttribute(
    "src",
    "./images/pokemon_sprites/" + pokemon[0] + ".png"
  );
...
document
  .getElementById("mobile-pokemon-info-button")
  .addEventListener("click", () => {
    document
      .getElementById("selected-pokemon")
      .scrollIntoView({ behavior: "smooth" });
  });

document
  .getElementById("mobile-pokemon-search-button")
  .addEventListener("click", () => {
    document
...
document.getElementById("pokemon-section").addEventListener("scroll", () => {
  if (
    document.getElementById("pokemon-section").scrollLeft >
    window.innerWidth / 2
  ) {
...
document
  .getElementById("selected-pokemon-data-window-select-button-right")
  .addEventListener("click", () => {
    document
      .querySelector(".selected-pokemon-data-stats-view")
      .scrollIntoView({ behavior: "smooth" });
  });

document
  .getElementById("selected-pokemon-data-window-select-button-left")
  .addEventListener("click", () => {
    document
      .querySelector(".selected-pokemon-data-info-view")
      .scrollIntoView({ behavior: "smooth" });
  });
...
document.querySelectorAll(".pokemon-entry").forEach((pokemonEntry) => {
  ...
  pokemonEntry.addEventListener("mouseenter", () => {
    pokemonEntry
      .querySelector("img")
      .classList.add("pokemon-entry-type-" + pokemonEntryType.dataset.type);
    pokemonEntryType.classList.add(
      "pokemon-entry-type-" + pokemonEntryType.dataset.type
    );
  });
  pokemonEntry.addEventListener("mouseleave", () => {
    pokemonEntry
      .querySelector("img")
      .classList.remove("pokemon-entry-type-" + pokemonEntryType.dataset.type);
    pokemonEntryType.classList.remove(
      "pokemon-entry-type-" + pokemonEntryType.dataset.type
    );
  });
});
...
document.querySelectorAll(".pokemon-entry").forEach((entry) => {
  entry.addEventListener("click", () => {
    ...
    document
      .getElementById("selected-pokemon-image")
      .setAttribute("src", "./images/pokemon_sprites/" + pokemon[0] + ".png");
    document
      .getElementById("selected-pokemon-image")
      .setAttribute("alt", pokemon[0]);
    document.getElementById("selected-pokemon-data-name").innerHTML =
      pokemon[1][0]["name"];
    ...
    document.getElementById(
      "selected-pokemon-data-info-content-national"
    ).innerHTML = "No. " + numberProcess;

    //Updates pokemon info type
    if (pokemon[1][0]["type"].length == 2) {
      let type1 =
        pokemon[1][0]["type"][0].charAt(0).toUpperCase() +
        pokemon[1][0]["type"][0].slice(1);
      document
        .getElementById("selected-pokemon-data-info-content-type-1")
        .querySelector("p").innerHTML = type1;
      document
        .getElementById("selected-pokemon-data-info-content-type-1")
      ...
      document
        ...
        .setAttribute(
          "src",
          "./images/type_icons/pokemon_type_icon_" +
            pokemon[1][0]["type"][0] +
            ".svg"
        );
      document
        .getElementById("selected-pokemon-data-info-content-type-1")
        .querySelector("img")
        .setAttribute("alt", pokemon[1][0]["type"][0] + " type icon");
      document
        .getElementById("selected-pokemon-data-info-content-type-1")
        .querySelector("div").style.backgroundColor =
        "var(--" + pokemon[1][0]["type"][0] + ")";

      let type2 =
        pokemon[1][0]["type"][1].charAt(0).toUpperCase() +
        pokemon[1][0]["type"][1].slice(1);
      document
        .getElementById("selected-pokemon-data-info-content-type-2")
        .querySelector("p").innerHTML = type2;
      document
        .getElementById("selected-pokemon-data-info-content-type-2")
        .querySelector("img").style.opacity = 1;
      document
      ...
      document
        .getElementById("selected-pokemon-data-info-content-type-1")
        .querySelector("img")
        .setAttribute("alt", pokemon[1][0]["type"][1] + " type icon");
      document
        .getElementById("selected-pokemon-data-info-content-type-2")
        .querySelector("div").style.backgroundColor =
        "var(--" + pokemon[1][0]["type"][1] + ")";
    } else {
      let type1 =
        pokemon[1][0]["type"][0].charAt(0).toUpperCase() +
        pokemon[1][0]["type"][0].slice(1);
      ...
      document
        .getElementById("selected-pokemon-data-info-content-type-2")
        .querySelector("img").style.opacity = 0;
      document
        .getElementById("selected-pokemon-data-info-content-type-2")
        .querySelector("div").style.backgroundColor = "var(--ui_dark_2)";
    }
    ...
    document.getElementById(
      "selected-pokemon-data-info-content-stat"
    ).innerHTML = statTotal;

    //Updates pokemon info debut generation
    document.getElementById(
      "selected-pokemon-data-info-content-generation"
    ).innerHTML = pokemon[1][0]["debut_generation"];

    //Updates pokemon stat values
    for (let stat = 0; stat < pokemon[1][0]["base_stats"].length; stat++) {
      let statPath = document.getElementById(
        "selected-pokemon-data-stat-" + stat
      );
      let baseStatValue = pokemon[1][0]["base_stats"][stat];
      statPath.querySelector(".selected-pokemon-data-stat-value").innerHTML =
        baseStatValue;
      let baseStatPercent = ((baseStatValue / 256) * 100).toFixed(0);
      statPath.querySelector(".selected-pokemon-data-stat-bar").style.width =
        baseStatPercent + "%";
      statPath.querySelector(
        ".selected-pokemon-data-stat-bar-fill"
      ).style.backgroundColor = "hsl(" + baseStatPercent * 2 + ", 75%, 45%)";
    }
  });
});
